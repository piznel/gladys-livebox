...
module.exports = function init() {
  ...
  return gladys.param.getValue('DECODEUR_LIVEBOX')
    .catch(function() {
      sails.log.error('please install this module again, sorry.');
    })
    .then((ip) => {
      if (ip !== '1.1.1.1' && ip.length > 0) {
        ...
        sails.log.error('please enter the IP of the decoder in the module configuration and then run a new configuration')
      }
    });
}
    ...
    getContent('http://' + ip + ':8080/remoteControl/cmd?operation=10')
      .then((result) => {
        ...
        if (typeof result.result.data.activeStandbyState !== 'undefined' && (result.result.data.activeStandbyState).length === 1) {
          ...
          // save power status
          shared.setStatusPower(tmpStatus);

          // then update the power devicetype
          var options = {
            deviceIdentifier: ip,
